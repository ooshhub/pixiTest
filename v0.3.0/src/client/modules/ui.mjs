...
/* UI COMPONENTS */
	// In-game menu
	// TODO: Create a UiAction class to bundle actions into
	// TODO: Create a keybindings data object to handle keyups
	const toggleGameMenu = () => {
		const menu = document.querySelector('section#ingamemenu');
		if (!menu) return;
		if (menu.children.length > 0) {
			...
			Game.LocalHub.trigger('main/requestHtml', { req: 'ingamemenu' });
		}
	}
	document.addEventListener('keyup', (ev) => {
		if (/esc/i.test(ev.key)) {
			let currentPage = window.sessionStorage?.activePage;
			if (currentPage === 'gamecanvas') toggleGameMenu();
		}
	});
});
